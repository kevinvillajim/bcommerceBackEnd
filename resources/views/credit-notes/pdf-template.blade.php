...
<body>
    <div class="invoice-container">
            ...
            <div class="header-top">
                ...
                <div class="invoice-title">
                    ...
                    <div class="invoice-meta">
                        <p><strong>No. {{ $creditNote->credit_note_number }}</strong></p>
                        <p><strong>Fecha:</strong> {{ $creditNote->issue_date->format('d/m/Y') }}</p>
                        <p><strong>Estado:</strong> {{ $creditNote->status === 'AUTHORIZED' ? 'Autorizada' : 'Pendiente' }}</p>
                    </div>
                </div>
            </div>
        </div>
        ...
        <div class="info-section">
            <div class="customer-info">
                <div class="info-block">
                    ...
                    <p><strong>Nombre:</strong> {{ $creditNote->customer_name }}</p>
                    <p><strong>{{ $creditNote->getCustomerIdentificationType() === '05' ? 'Cédula' : 'RUC' }}:</strong> {{ $creditNote->customer_identification }}</p>
                    <p><strong>Email:</strong> {{ $creditNote->customer_email }}</p>
                    <p><strong>Dirección:</strong> {{ $creditNote->customer_address }}</p>
                    @if($creditNote->customer_phone)
                        <p><strong>Teléfono:</strong> {{ $creditNote->customer_phone }}</p>
                    @endif
                </div>
            </div>
            ...
            <div class="invoice-details">
                <div class="info-block">
                    ...
                    <p><strong>Tipo:</strong> {{ $creditNote->documento_modificado_tipo === '01' ? 'Factura' : 'Documento' }}</p>
                    <p><strong>Número:</strong> {{ $creditNote->documento_modificado_numero }}</p>
                    <p><strong>Fecha:</strong> {{ $creditNote->documento_modificado_fecha ? $creditNote->documento_modificado_fecha->format('d/m/Y') : 'N/A' }}</p>
                    @if($creditNote->invoice)
                        <p><strong>Factura Original:</strong> {{ $creditNote->invoice->invoice_number }}</p>
                        <p><strong>Monto Original:</strong> ${{ number_format($creditNote->invoice->total_amount, 2) }}</p>
                    @endif
                </div>
            </div>
        </div>
        ...
        <div class="credit-note-reason">
            <div class="reason-title">Motivo de la Nota de Crédito</div>
            <div class="reason-text">{{ $creditNote->motivo }}</div>
        </div>
                ...
                <tr>
                    ...
                    <td>
                        ...
                        @if($item->product_code)
                            <div class="item-sku">Código: {{ $item->product_code }}</div>
                        @endif
                    </td>
        ...
        @if($creditNote->status === 'AUTHORIZED')
        <div style="text-align: center; margin-bottom: 15px; font-size: 11px;">
            ...
            <p style="color: #555555; font-size: 9px;">
                Fecha de autorización: {{ $creditNote->updated_at->format('d/m/Y H:i:s') }}
            </p>
        </div>

        <!-- SRI Information -->
        <div class="sri-section">
            <h4>Información de Autorización - SRI</h4>
            <div class="sri-info">
                <div>
                    <p><strong>Clave de Acceso:</strong></p>
                    <p style="font-size: 9px; word-break: break-all;">{{ $creditNote->sri_access_key }}</p>
                    @if($creditNote->sri_authorization_number)
                    <p><strong>Número de Autorización:</strong></p>
                    <p style="font-size: 9px;">{{ $creditNote->sri_authorization_number }}</p>
                    @endif
                </div>
                <div>
                    <p><strong>Estado:</strong> AUTORIZADO</p>
                    <p><strong>Fecha de Autorización:</strong> {{ $creditNote->updated_at->format('d/m/Y H:i:s') }}</p>
        ...
        <div style="margin-bottom: 15px; padding: 8px; background: #f8f9fa; border-left: 3px solid #0f497d; font-size: 9px;">
            ...
            <p>• Esta nota de crédito modifica la factura {{ $creditNote->documento_modificado_numero }}</p>
            <p>• El valor de esta nota de crédito debe ser aplicado como crédito a favor del cliente</p>
            <p>• Documento electrónico válido conforme a la normativa del SRI</p>
        </div>
        ...
        <div class="footer">
            ...
            <p>Este documento constituye una nota de crédito electrónica válida conforme a la normativa ecuatoriana vigente</p>
            ...
            <p style="margin-top: 8px; color: #d63031; font-weight: bold;">
                IMPORTANTE: Esta nota de crédito está pendiente de autorización por parte del SRI
            </p>
            @endif
        </div>
    </div>
